...
public class Aerolinea
{
    ...
    {
    	for (Vuelo i : vuelos) {
            if (i.getRuta().getCodigoRuta().equals(codigoRuta) && i.getFecha().equals(fechaVuelo)) {
                return i;
            }
        }
        return null;
    ...
    public Collection<Tiquete> getTiquetes() {
        ...
        for (Vuelo vuelo : vuelos) {
            todosLosTiquetes.addAll(vuelo.getTiquetes().values());
        }
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        for (Vuelo vuelo : vuelos) {
            if (vuelo.getAvion().getNombre().equals(nombreAvion) && vuelo.getFecha().equals(fecha)) {
                throw new Exception("El avión " + nombreAvion + " ya está ocupado en la fecha " + fecha);
            }
        }
    }
    ...
    {
    	Ruta ruta= ruta.getCodigoRuta(codigoRuta);
    	Collection<Tiquete> tiquetes= this.getTiquetes(); 
    ...
    public void registrarVueloRealizado( String fecha, String codigoRuta )
    {
    	...
    	for(Vuelo i: vuelos) {
    		if(i.getFecha().equals(fecha) && i.getRuta().getCodigoRuta().equals(codigoRuta)) {		
    			System.out.println("Se realizo el vuelo");
    		}
    	}
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente ) {
        ...
        List<Tiquete> tiquetesSinUsar = cliente.tiquetesSinUsar;
